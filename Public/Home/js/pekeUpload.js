    ...
    $.fn.pekeUpload = function(options) {
        ...
        var pekeUpload = {
            ...
            handlebuttonevents: function() {
                //前台上传处理
                if (options.locations=='index'){
                    $(document).find('.formDiv').find('#file').on("change", this.obj.selector, function() {
                        //alert((pekeUpload.obj[0].files[0]));
                        pekeUpload.checkFile(pekeUpload.obj[0].files[0]);
                    });
                    $(document).find('.formDiv').on('click','.pkdel',function(){
                        var parent = $(this).parent('div').parent('div');
                        pekeUpload.delAndRearrange(parent);
                    });
                }else if(options.locations=='face'){
                    //处理头像上传
                    $(document).find('#faceUpload').find('#file').on("change", this.obj.selector, function() {
                        //alert((pekeUpload.obj[0].files[0]));
                        pekeUpload.checkFile(pekeUpload.obj[0].files[0]);
                    });
                    $(document).find('#faceUpload').find('#file').on('click','.pkdel',function(){
                        var parent = $(this).parent('div').parent('div');
                        pekeUpload.delAndRearrange(parent);
                    });
                }
            ...
            previewFile: function(container, file) {
                ...
                switch (type) {
                  case "image":
                    ...
                    var prev = $('<img class="thumbnail" src="' + ThinkPHP['IMG'] + '/loading_4.gif"  />').appendTo(container);
                    break;
                    ...
                    success: function(data) {
                        ...
                        if (data == 1 || $.parseJSON(data).success == 1) {
                            ...
                            if (error in $.parseJSON(data)) {
                                ...
                                err = $.parseJSON(data).error;  //将错误信息传递到前台
                            }
                            if (options.showErrorAlerts) {
            ...
            delAndRearrange: function(parent){
                ...
                $('.formDiv').find('input[rel="'+id+'"]').remove();  //自定义
                parent.remove();
                pekeUpload.container.find('div.pkrw').each(function(index){
                    $(this).attr('rel',index);
                });
                //自定义，重新排序input
                $('.formDiv').find('input[class=srcs]').each(function (index) {
                    $(this).attr('rel',index);
                })
            }
        };
